...
Provided that you have setup your project with new versions of dermanager (dermanger >= 0.4.3) the script `commands/run_copy_number.sh` should have been autopopulated. If this is the case then you should be able to launch the pipeline on the Sanger farm22 LSF with:
...
Once you have successfully run the pipeline and are satisified that the results are unlinkely to change please tidy your work area by removing the intermediate files in the `work` directory.
